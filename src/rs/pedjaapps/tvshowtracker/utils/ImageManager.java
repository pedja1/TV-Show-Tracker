...
public class ImageManager
{
    ...
    {
        if(LOGGING)Log.e(Constants.LOG_TAG, ">>> ImageManager Constructor");
        ...
        if(LOGGING)Log.v(Constants.LOG_TAG, "cacheDir: [" + cacheDirImages + "]");
    ...
    public void getWebImage(ImageReference imageReference)
    {
        ...
        else if (webImageMap.containsKey(imageReference.getImageName()))
        {
            ...
            if (bitmap != null)
            {
                ...
                {
                    if(LOGGING) Log.e(Constants.LOG_TAG, "getWebImage: bitmap was recycled: [" + imageReference.getImageNameHumanReadable() + "]");
                }
            } else
            {
                if(LOGGING)Log.w(Constants.LOG_TAG, "getWebImage: found in map with no pic: [" + imageReference.getImageNameHumanReadable() + "]");
            }
        }
...
// **************************************************
    private int computeSampleSize(InputStream stream, ImageReference imageReference)
    {
        ...
        if(LOGGING)Log.v(Constants.LOG_TAG, "image original w|h: " + sourceWidth + "|" + sourceHeight);
        if (sourceWidth < 1 || sourceHeight < 1)
        {
            if(LOGGING) Log.e(Constants.LOG_TAG, "invalid image dimensions: " + imageReference.getImageNameHumanReadable());
        }
        ...
        if(LOGGING)Log.d(Constants.LOG_TAG, "ImageManager imageSize = " + sampleSize);
        return sampleSize;
    }
    ...
    private void imageDimensions(int sourceWidth, int sourceHeight, ImageReference imageReference)
    {
        ...
        if(LOGGING)Log.v(Constants.LOG_TAG, "rescaled w|h: " + imageReference.scaleToWidth + "|" + imageReference.scaleToHeight);
    }
    ...
    {
        String myFilename = urlIntoFileName(imageReference);
        File myFile = new File(cacheDirImages, myFilename);

        if(LOGGING) Log.e(Constants.LOG_TAG, "ImageManager :: getBitmap(): " + imageReference.getImageNameHumanReadable());
        ...
        if (bitmap != null)
        {
            if(LOGGING)Log.i(Constants.LOG_TAG, "ImageManager :: got image from cache: [" + imageReference.getImageNameHumanReadable() + "]");
            return bitmap;
        }
        // Nope, have to download it
        else
        {
            if(LOGGING)Log.d(Constants.LOG_TAG, "ImageManager :: downloading: [" + imageReference.getImageNameHumanReadable() + "]");
            try
            {
                ...
                {
                    if(LOGGING)Log.d(Constants.LOG_TAG, "ImageManager writing cache: [" + myFile + "]");
            ...
            {
                if(LOGGING)Log.e(Constants.LOG_TAG, "ImageManager :: getBitmap :: error: " + ex.getMessage());
    ...
    public static String urlIntoFileName(ImageReference imageReference)
    {
        String url = imageReference.getImageName();
        String result = "";

        String[] ext = url.split("\\.");

        String extension = ext[ext.length - 1];
        if(imageReference.isMap) extension = "png";
        if (extension.length() != 3 && extension.length() != 4)
        {
            if(LOGGING)Log.e(Constants.LOG_TAG, "unexpected extension: " + extension);
        }
...
// **************************************************
    public static class ImageReference
    {
        ...
        private int loadingImageIdAlt = R.drawable.ic_launcher;
        ...
        public boolean isMap;// url for static map isnt absolute image url o it causes problems.
        // If this is true ".png" extention will be assigned to filename
       ...
       /* public ImageReference(String imageURI, ImageView imageView, PhotoViewAttacher attacher)
        {
            this.imageURI = imageURI.trim();
            this.imageView = imageView;
            this.loading = false;
        ...
        /*public String getImageNameForMap(String locationTag)
        {
            String widthTag = "" + this.scaleToWidth;
            if (doScaleInPercent())
            {
            ...
            return "" + locationTag + this.doScale() + widthTag + this.doScaleInPercent() + this.imageURI;
        }*/
    ...
    private class BitmapDisplayer implements Runnable
    {
        ...
        public void run()
        {
            if (bitmap != null)
            {
                ...
                /*if (imageReference.attacher != null)
                {
                    imageReference.attacher.update();
                }*/
            } else
            {
                 imageReference.getImageView().setImageResource(R.drawable.noimage);
